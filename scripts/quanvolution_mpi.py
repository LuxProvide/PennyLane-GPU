...
#    cpu_time = time_process(lambda: q_process(train_images, circuit_cpu, "lightning.qubit"))
    ...
    print(f"Completed: Wires={wires}, Layers={layers}, GPU time={gpu_time:.4f}s")
    return gpu_time

# Parameters to test
wires_range = [10, 15, 18, 20, 25, 33]
layers_range = [3]
num_runs = 1
...
for i, wires in enumerate(wires_range):
    for j, layers in enumerate(layers_range):
        gpu_time = run_experiment(wires, layers, num_runs)
...
for j, layers in enumerate(layers_range):
    ...
    ax.set_title(f'(one)GPU Timing Results (Layers={layers})')
    ax.set_xticks(x)
    ...
    filename = f'gpu_timing_layers_{layers}.png'
